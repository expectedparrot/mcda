...
from mcda.core.project import find_project
from mcda.core.store import list_entities, list_records
...
def _recommendation(state: dict) -> tuple[str, str]:
    ...
    if not counts["performance_records"] and not counts["assessments"]:
        return "mcda assessment build --id round_1", "Generate EDSL Jobs for agent-based performance elicitation."
    if counts["assessments"] and not counts["performance_records"]:
